...
######### generic parsing part ##########
    ...
    def do_sample_default(self,event):
        process = self.generic_find_process(event.tid, event.comm+":"+event.event,"sample")
        self.generic_process_single_event(process,event)
        process['comments'].append(event.event)
    ...
    def handle_sample_event(self,event):
        self.linenumbers.append(event.linenumber)
        self.timestamps.append(event.timestamp)
        self.run_callbacks("do_all_samples", event)
        if not self.run_callbacks("do_sample_"+event.event, event):
            self.do_sample_default(event)
